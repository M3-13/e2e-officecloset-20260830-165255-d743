...
def env(tmp_path):
    ...
    The auth and item-management tickets are still stubs on this branch, so
    ``get_current_user`` is overridden with a real, known user and ``get_db``
    with a session on an in-memory database. This keeps the test independent of
    the sibling tickets while exercising the account router for real.
    """
    ...
    (upload_dir / "a.jpg").write_bytes(b"image-a")
    (upload_dir / "b.jpg").write_bytes(b"image-b")
    ...
    yield user_id, upload_dir, testing_session
...
def test_delete_account_removes_user_items_outfits_and_images(env) -> None:
    user_id, upload_dir, testing_session = env
...
def test_login_with_old_credentials_fails_after_deletion(env) -> None:
    ...
    nothing — which is what makes a subsequent login answer 401."""
    user_id, _upload_dir, testing_session = env
